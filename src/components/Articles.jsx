...
/* next iteration of page 
   Image, Title, Comments links will navigate to SingleAritcle component with article id in url
...
*/
        ...
        return (
            ...
            <div>
            <a href='#'><img src={article_img_url} alt={title} ></img></a>
            </div>
            <div>
            <p>Title: <a href="#">{title}</a></p>
            <p>Author: <a href="#">{author}</a></p>
            <p>Category: <a href="#">{topic}</a></p>
            <p>Posted: {created_at.substring(0,10)}</p>
            <p>Votes: {votes}</p>
            <p>Comment: <a href='#'>{comment_count}</a></p>
            </div>
            </section>
        )
    })
